...
class ASTArrayElementLoad : public RecursiveASTVisitor<ASTArrayElementLoad> {
public:
  bool VisitArraySubscriptExpr(ArraySubscriptExpr *ASE) {
    elements.emplace(ASE);
    ...
    while(1) {
      ...
      if(ArraySubscriptExpr *ase = dyn_cast<ArraySubscriptExpr>(base)) {
        elements.emplace(ase);
        base = ase->getBase();
        continue;
      }
  ...
  const std::set<ArraySubscriptExpr *> &getArrayElements() { 
    return elements; 
  }
private:
  std::set<ArraySubscriptExpr *> elements;
};

class ASTFieldVariableLoad : public RecursiveASTVisitor<ASTFieldVariableLoad> {
public:
  bool VisitMemberExpr(MemberExpr *ME) {
    fieldVariables.emplace(ME);
    return true;
  } 
  const std::set<MemberExpr *> &getFieldVariables() { 
    return fieldVariables; 
  }
private:
  std::set<MemberExpr *> fieldVariables; 
};
...
public:
  ...
  bool VisitVarDecl(VarDecl *VD) {
    if (isFunctionPointer(VD)) {
      auto it = fdTypes.find(VD->getFunctionType());
      if (it != fdTypes.end())
      {
      ...
      if (VD->hasInit()) {
        ...
        for (DeclRefExpr *DRE : DREs) {
          ...
          // }
          //其他函数指针赋值
          if (VarDecl *right = dyn_cast<VarDecl>(DRE->getDecl())) {
      ...
      if (VarDecl *VD = dyn_cast<VarDecl>(DRE->getDecl())) {
        ...
        if (isFunctionPointer(VD)) {
          ...
          for (DeclRefExpr *DRE : DREs) {
            ...
            // }
            //函数指针间赋值
            if (VarDecl *var = dyn_cast<VarDecl>(arguement)) {
  ...
  ASTFunctionPtrLoad(FuncPtrInfo &mayPointTo, FunctionDecl *FD, std::unordered_map<const FunctionType *, std::vector<FunctionDecl *>> fdtypes)
      : pointTo(mayPointTo), parent(FD), fdTypes(fdtypes)
  {
    for (auto it_param = parent->param_begin(); it_param != parent->param_end(); it_param++)
    {
      ParmVarDecl *param = *it_param;
      //处理函数的函数指针参数
      if (isFunctionPointer(param))
      {
        auto it = fdTypes.find(param->getFunctionType());
        if (it != fdTypes.end())
        {
          for (size_t i = 0; i < (*it).second.size(); i++)
          {
            auto fd = (*it).second[i];
            addPointToInfo(param, fd, parent);
          }
        }
      }
    }
...
std::vector<std::pair<int64_t, std::set<std::string>>> getFunctionPtrWithCS(
  ...
  // std::set<std::string>>>;
  ASTFunctionPtrLoad load(mayPointTo, FD,fdTypes);
...
std::vector<MemberExpr *> getFieldVariables(FunctionDecl *FD) {
  std::vector<MemberExpr *> variables;

  ASTFieldVariableLoad load;
  load.TraverseStmt(FD->getBody());
  variables.insert(variables.end(), load.getFieldVariables().begin(),
...
std::vector<ArraySubscriptExpr *> getArrayElements(FunctionDecl *FD) {
  std::vector<ArraySubscriptExpr *> elements;

  ASTArrayElementLoad load;
  load.TraverseStmt(FD->getBody());
  elements.insert(elements.end(), load.getArrayElements().begin(),
                   load.getArrayElements().end());
...
void common::printLog(std::string logString, common::CheckerName cn, int level,
  ...
  case common::CheckerName::arrayBound:
    ...
    break;
  case common::CheckerName::LoopUpbound:
    if (block.find("LoopUpbound")->second == "true" && level >= l) {
      llvm::errs() << logString;
    }
    break;
  case common::CheckerName::RedundantJudgement:
    if (block.find("RedundantJudgement")->second == "true" && level >= l){
      llvm::errs() << logString;
    }
    break;
  }
